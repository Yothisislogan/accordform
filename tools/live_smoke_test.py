...
The licensed templates are NOT stored in git, so this is meant to run on the
box that has them. Each form needs its clean template at
templates/acord/ACORD_<number>_clean.pdf (run tools/prep_template.py first).
Missing templates are reported, not faked.
...
Usage:
    ...
    python tools/live_smoke_test.py --form 25 --no-flatten
"""
...
# Deterministic sample value per field type, so outputs are easy to spot.
SAMPLE = {
...
def smoke_one(schema_path: Path, out_dir: Path, flatten: bool = True) -> tuple[str, bool, str]:
    ...
    try:
        _, result = produce_pdf(schema, template, sample_answers(schema), out, flatten=flatten)
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--no-flatten", action="store_true", help="skip pdftk flatten (faster; editable output)")
    ...
    ok_count = 0
    for path in targets:
        number, ok, msg = smoke_one(path, out_dir, flatten=not args.no_flatten)
